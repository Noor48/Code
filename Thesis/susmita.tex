...
with
    \begin{equation}
        \overline{G} _{M,i} = RT ln a_i       \tag*{(A short note)} 
    \end{equation}
...
\begin{adjustbox}{center,caption={FiG},label={somelabel},nofloat=figure,vspace=\bigskipamount}
    \includegraphics[width=0.7\textwidth]{f-1}
\end{adjustbox}

    \subsection*{Figure-1:}
    A schematic diagram symbolizing the Gibbs energy of mixing at constant T plotted against concentration. 
    Curve a, complete mixing ($$T$$ > Tc). 
    Curve b, incomplete mixing(T2 < Tc), 
    $\Delta c$ represents the miscibility gap at T2 .
\leqslant 
...
phases are equivalent,
    ...
    \begin{equation}
        \overline{G} _{M,i}(c _{1}) =  \overline{G} _{M,i}(c _{2})       (i= A,B).
    \end{equation}
...
with
    ...
    \subsection*{Perceptible indicators}
